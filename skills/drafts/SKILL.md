...
2. Spawn N variants on the panel from `/setup-jstack` (default 3). Split the `challenge panel` line on commas; one variant per entry. Each variant is a full draft, not a bullet list of tones. Use `subagent_type: "jstack-panel-<number>"` for each member. If `/setup-jstack` has never run, use `subagent_type: "jstack-agent"` for all 3 members.
