...
2. Spawn N variants on the panel from `/setup-jstack` (default 3). Split the `challenge panel` line on commas; one variant per entry. Each variant is a full draft, not a bullet list of tones. Always `subagent_type: "jstack-agent"`; put each panel entry on Task `model` (omit `model` for `inherit`).
