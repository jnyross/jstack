...
2. Fan out on a sweep model. GBrain `query`, `get_page`, `get_recent_salience`, `recall`. Live Calendar `list_events`. Live Gmail `search_threads`. Linear only if the topic is work.
